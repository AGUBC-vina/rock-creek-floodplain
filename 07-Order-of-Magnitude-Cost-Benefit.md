...
> **MAJOR SIMPLIFICATION — to be verified.** Updated landowner intel: **the project area has only 4 landowners total.** The **major landowner holds ~9 mi (~87%) of the ~10.4 mi corridor** under an existing NRCS conservation easement. Three smaller landowners cover the remaining ~1.4 mi: 1–2 at the diversion intake on Rock Creek, 1 along a segment of the south route, 1 along a segment of the north route. If verified, this changes three things materially:
>
> 1. **Land-control cost (Section 4.1, line G) drops from ~$2 M to ~$1–1.5 M.** Instead of negotiating new easements with many family trusts, the work is: (a) one major-landowner partnership + NRCS easement amendment / supplemental management agreement for the ~9 mi NRCS-easement reach, and (b) three smaller-scope easement negotiations on the remaining segments. **Total capital could fall from ~$30 M to ~$28–29 M central.**
> 2. **NRCS becomes a load-bearing permitting partner for ~87% of the corridor.** The existing easement has specific protection purposes (likely meadowfoam / vernal pool / general habitat). The Rock Creek concept must be **compatible** — for *habitat enhancement* this is usually achievable through an easement amendment, but the supplemental agreement must be negotiated early and is a critical-path item.
> 3. **The mitigation-bank "additionality" question (Section 5.5) gets thornier on the NRCS reach.** Baseline conservation value on ~9 mi is already "captured" by the existing easement, so Path A (full bank) becomes even less viable for that reach. The non-NRCS segments (~1.4 mi, three landowners) could in principle support bank credits if their habitat is incremental — but the area is small and probably not worth the establishment cost. Paths B (single-buyer Caltrans) and C (ILF) still work corridor-wide because they pay for *enhancement uplift above baseline*, not for baseline preservation.
>
> **Risk-profile implication:** the major landowner is now a **go/no-go partner for ~87% of the project**. A "yes" unlocks both phases simultaneously (north + south) on the easement land. A "no" blocks the bulk of the concept, leaving only ~1.4 mi of corridor on the three smaller landowners — likely not viable at scale. The three smaller landowners are individually segment-blockers: each "no" reduces project scope by their segment without killing the whole concept.
>
> Verification path: confirm via Butte County Assessor records + NRCS easement records + the four landowner conversations (Roadmap Steps L-1, L-3, L-4 — see updated language in Deliverable 04 Section 3). This is the **highest-value early-work item** because both cost and timeline collapse if confirmed.
...
If $8M one-time falls outside grants, who absorbs it? The beneficiary list (in rough proportion to benefit received). **Note:** if the 4-landowner / major-NRCS-easement intel above is confirmed, the **major landowner is a key project partner** (and may contribute through in-kind value such as easement co-management, parcel access, or staff time) rather than a paid party for ~87% of the corridor. The three smaller landowners may be paid easement holders or in-kind partners depending on negotiation outcome. Beneficiary allocation below assumes the major landowner is an in-kind partner.
...
1. **Land-control structure** *(newly elevated — see Section 5 callout).* If the 4-landowner / major-NRCS-easement intel is confirmed, **two things change simultaneously**: land-control cost drops by ~$0.5–1M, and the landowner-coordination critical path collapses from "many family-trust negotiations" to one major NRCS-amendment conversation plus three smaller-scope easement discussions. The major landowner becomes a go/no-go partner for ~87% of the corridor. This is **the highest-value early-work verification** — possibly more impactful than the FDEM survey because it changes both cost and timeline.
